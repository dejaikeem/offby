...
- **야간 대규모 run (9/10, 30B, 5,569콜, 6시간, $0, `docs/experiments/2026-09-10-overnight-30b/`):** 1,351콜 건강 run에서 **오탐 0** · 4,001콜 think-off 완주 · 심판 판정 시간 5,569콜에서 3.9ms(선형, 1만 콜까진 무시 가능). **그런데 긴 프롬프트로 바꾼 급변(평균 2.2×)을 217콜 동안 못 잡았다** — 꼬리가 길어 t > 2.5 확신에 못 미침. 급변이 3배 이상이면 10~15콜에 잡지만 2~3배의 지속적 초과엔 무력. CUSUM을 병행하면 같은 데이터에서 168콜에 잡힘 → **결정 필요**(8절).
...
2. ~~야간 배치 + 급변 주입~~ 완료 (5,569콜; 위 5절) → CUSUM 병행 결정
3. Nebius 키 → Token Factory 실측 (해커톤 전제)
4. 밋업(9/11) 후: alert 모드·웹훅 → sticky halt·예산 강제 → O(1) 심판 → LiteLLM 플러그인 스파이크

**결정 필요:** (1) **t-검정에 CUSUM을 병행할 것인가** — 야간 run이 보여준 대로 t-검정만으로는 2~3배의 지속적 초과를 못 잡는다(2.2×를 217콜 동안 통과시킴); CUSUM 병행이면 168콜에 잡히고 오탐 0은 유지될 것으로 보이지만 시뮬로 확인 필요. (2) halt 기본값을 alert로 바꿀 것인가. (3) 해커톤 트랙(Best Apps & Agents 유지). (4) ~~레포 공개 시점~~ — 2026-09-11 공개 전환 완료(Apache-2.0, 이력 감사: 비밀·이메일·회사 언급 없음).
