...
**결정 필요:** (1) **t-검정에 CUSUM을 병행할 것인가** — 야간 run이 보여준 대로 t-검정만으로는 2~3배의 지속적 초과를 못 잡는다(2.2×를 217콜 동안 통과시킴); CUSUM 병행이면 168콜에 잡히고 오탐 0은 유지될 것으로 보이지만 시뮬로 확인 필요. (2) halt 기본값을 alert로 바꿀 것인가. (3) 해커톤 트랙(Best Apps & Agents 유지). (4) 레포 공개 시점(지금 private).
